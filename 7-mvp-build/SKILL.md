...
필수 산출물:
- `design-system/` 폴더 — 디자인 토큰, 컴포넌트 상세 스펙 (필요한 파일만 한 번씩 read)
- `design-preview/screens/` — 화면 시안 (작업할 화면만 read)
- `ux-flow/01-user-flow.md` — 화면 간 이동 (한 번만 read)
- `ux-flow/06-test-cases.md` — 검증 시나리오 (마지막 단계에 read)

`ux-audit-report.md`가 있으면 감사 이슈 반영 (한 번만 read).

**토큰 절약**: 필요한 파일을 **딱 한 번씩만** read. 대화 중 같은 파일 반복 read 금지.
...
- 진행 상태에서 7단계 ✅로 표시
- 산출물 인덱스에 MVP 코드 경로 추가
- 핵심 결정사항에 기술 스택 (프레임워크, 스타일링, 라우팅, 빌드 도구) 기록
- 자주 쓰는 명령어(설치/실행/빌드/테스트)를 CLAUDE.md에 별도 섹션으로 추가
- 구현 중 디자인이나 흐름에 변경이 일어난 경우 의사결정 로그에 기록
