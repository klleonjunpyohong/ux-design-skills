...
컴포넌트를 **아토믹 디자인** 5계층으로 분류해 정리한다:

**Atoms (원자)** — 더 이상 쪼갤 수 없는 최소 단위
- 버튼, 입력 필드, 라벨, 아이콘, 배지/태그, 체크박스, 라디오, 스위치, 아바타

**Molecules (분자)** — Atoms를 조합한 단순 단위
- 검색바 (입력+아이콘+버튼), 폼 필드 (라벨+입력+에러 메시지), 카드 헤더, 토스트/알림

**Organisms (유기체)** — Molecules를 조합한 복합 단위
- 네비게이션 바 (상단/하단), 카드 (전체), 리스트 아이템, 모달/팝업, 폼, 헤더

**Templates (템플릿)** — 화면의 뼈대 (콘텐츠 없음)
- 페이지 레이아웃 (헤더+본문+푸터), 그리드 시스템

**Pages (페이지)** — 실제 화면 (6단계에서 다룸)

`ux-flow.md`의 공통 컴포넌트와 화면별 UI 구조에서 추출하여 위 계층으로 분류. 사용자에게 분류된 목록을 보여주고 확인.
...
- 진행 상태에서 5단계 ✅로 표시
- 산출물 인덱스에 `design-system.md`, `design-preview/` 추가
- 핵심 결정사항에 디자인 컨셉 키워드, Primary 컬러, 폰트 패밀리 기록
- 사용자 피드백으로 컬러/폰트/컴포넌트가 변경된 경우 의사결정 로그에 기록
